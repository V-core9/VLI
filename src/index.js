...
const vLog = {
  ...
  printMessage(logItem) {
    ...
    var styledMessage = `\n⌘  ${logItem.msg} \n\n${chalk.gray("⏰ " + logItem.ts)}`;

    if (config.exec_times === true) styledMessage = `${styledMessage}\nExec: ${logItem.exec}`;

    switch (logItem.type) {
      case "log":
        styledMessage = boxen(chalk.rgb(50, 150, 50)(styledMessage), { title: chalk.bold.white.bgRgb(50, 150, 50)(` ⟁ Msg_UID : ${logItem.uid} `), padding: { top: 0, left: 2, right: 2, bottom: 0 } });
        break;
      case "info":
        styledMessage = boxen(chalk.rgb(50, 150, 210)(styledMessage), { title: chalk.bold.white.bgRgb(10, 100, 200)(` ⌆ Info_UID : ${logItem.uid} `), padding: { top: 0, left: 2, right: 2, bottom: 0 } });
        break;
      case "warn":
        styledMessage = boxen(chalk.rgb(225, 100, 50)(styledMessage), { title: chalk.bold.white.bgRgb(225, 100, 50)(` ⌬ Warn_UID : ${logItem.uid} `), padding: { top: 0, left: 2, right: 2, bottom: 0 } });
        break;
      case "error":
        styledMessage = boxen(chalk.rgb(250, 50, 50)(styledMessage), { title: chalk.bold.white.bgRgb(180, 60, 60)(` ⮽ Error_UID : ${logItem.uid} `), padding: { top: 0, left: 2, right: 2, bottom: 0 } });
        break;
